...
𝑃𝐼𝑁𝐺 ↬ {url2} 𝘔𝘴
"""
                            if os.path.exists('Mode/Status'):
                                matn = open('Mode/Status').read()
                            else:
                                open('Mode/Status', 'w').write('CIPHER-X')
                            if matn == textApp:
                                await event.reply(help_text)
                            else:
                                await event.reply(help_text + '\n\n🔹 یه پیام از طرف ادمین داری! 🔹\n\n برای خواندن پیام دستور \n-> .msg\n⭕ رو ارسال کنید ⭕')
                        except:
                            pass
    ...
    """)
                    ...
                    if mode == 'on':
                        ...
                        if time_old == time:
                            ...
                            draw.text((158, 480),f"~~ {time} ~~", font=font)
